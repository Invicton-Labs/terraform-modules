...
resource "aws_cloudwatch_log_subscription_filter" "subscription" {
  count           = var.create && var.subscription_lambda_arn != null ? 1 : 0
  name            = var.subscription_name
  log_group_name  = aws_cloudwatch_log_group.loggroup[0].name
...
resource "aws_lambda_permission" "allow_cloudwatch" {
  ...
  source_arn    = aws_cloudwatch_log_group.loggroup[0].arn
}
